...
const styles = StyleSheet.create({
  ...
  infoGrid: { flexDirection: 'row', flexWrap: 'wrap', backgroundColor: '#f3f4f6', padding: 15, borderRadius: 8, marginBottom: 20 },
  infoItem: { width: '50%', marginBottom: 5 },
  infoLabel: { fontWeight: 'bold', color: '#4b5563' },
...
export const MovimientosCajaPdf: React.FC<{ data: any }> = ({ data }) => (
  <Document>
    <Page size="A4" style={styles.page}>
      <View style={styles.header}>
        <Text style={styles.title}>Reporte de Movimientos de Caja</Text>
        <Text style={styles.subtitle}>Kantuta POS - Control de Caja Físico</Text>
      </View>
      <View style={styles.infoGrid}>
        <View style={styles.infoItem}><Text><Text style={styles.infoLabel}>Caja: </Text>{data.sesion.cajaNombre}</Text></View>
        <View style={styles.infoItem}><Text><Text style={styles.infoLabel}>ID Sesión: </Text>{data.sesion.id}</Text></View>
        <View style={styles.infoItem}><Text><Text style={styles.infoLabel}>Apertura: </Text>{new Date(data.sesion.fechaApertura).toLocaleString()}</Text></View>
        <View style={styles.infoItem}><Text><Text style={styles.infoLabel}>Cierre: </Text>{data.sesion.fechaCierre ? new Date(data.sesion.fechaCierre).toLocaleString() : 'SESIÓN ABIERTA'}</Text></View>
        <View style={styles.infoItem}><Text><Text style={styles.infoLabel}>Monto Inicial: </Text>Bs. {data.sesion.montoInicial.toFixed(2)}</Text></View>
        <View style={styles.infoItem}><Text><Text style={styles.infoLabel}>Saldo Actual: </Text>Bs. {data.sesion.saldoActualTeorico.toFixed(2)}</Text></View>
      </View>
      ...
      <View style={styles.totalBox}>
        ...
        <View style={[styles.totalRow, styles.totalFinal]}><Text>Total Neto:</Text><Text>Bs. {data.totales.neto.toFixed(2)}</Text></View>
      </View>
    </Page>
  </Document>
);
